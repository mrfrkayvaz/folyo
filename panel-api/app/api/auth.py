"""Panel giriş/giriş kontrolü — yalnızca admin kullanıcılar token alabilir.

Doğrulama, panel-api'nin şema sahibi olduğu `users` tablosu üzerinden SQLModel
modelleriyle yapılır (bcrypt hash uyumlu). Başarılı + admin ise HMAC imzalı
bearer token döner; panel frontend bu token'ı saklar ve tüm isteklere
`Authorization` header'ı olarak ekler (`require_auth` dependency'si doğrular).
"""
...
async def login(body: LoginRequest):
    ...
    if user.user_type != UserType.admin:
        raise HTTPException(403, "Panele yalnızca admin kullanıcılar giriş yapabilir.")
