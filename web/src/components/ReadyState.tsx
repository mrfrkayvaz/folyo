...
import { DEFAULT_CHAT_TITLE } from "../constants/index"
...
export default function ReadyState({ totalCount, attachments = [], onAsk, workspace }: ReadyStateProps) {
  const docs = attachments.filter((a) => a.phase === DocumentStatus.EMBEDDED && a.docId)
  const hasWsTitle = Boolean(workspace?.name && workspace.name !== DEFAULT_CHAT_TITLE)
