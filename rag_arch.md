...
| 15.09.2026 | **Dense skorları neden hep düşüktü → kök neden: Chroma uzayı `l2`'ydi!** `get_or_create_collection` mevcut koleksiyona `hnsw:space=cosine` metadata'sını uygulamıyor; koleksiyon eski bir kurulumdan **l2** uzayında kalmıştı → `score = 1 − distance = 1 − L2²` **negatif** değerler üretiyordu ve `dense ≥ 0.38` eşiği hiç geçilemiyordu (dense kanal ölü, her şeyi BM25 taşıyordu). Ölçüm kanıtı: `chroma_dist = 1.3282 = L2²`, gerçek kosinüs +0.3359. **Düzeltme:** `_col()` artık uzayı doğrular, `l2` ise koleksiyonu cosine ile yeniden yaratır (uyarı loglar; yeniden embed gerekir); ek olarak **embed/gösterim ayrımı** (`Chunk.embed_text` — breadcrumb ön eki embed'a girmez, LLM bağlamına `breadcrumbs` metadata'sından kurulur) + sembol/tire temizliği. Koleksiyon şu an `space=cosine`, skorlar gerçek kosinüs (+0.31…+0.34); `guard_dense_min` 0.38 → **0.30** kalibre edildi. |
