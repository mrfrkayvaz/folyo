...
SYSTEM_PROMPT = (
    ...
    "6. Görsel içerikli bir parçadan yararlandıysan, görselin anlatıldığı EN alakalı tek "
    "noktaya satır içi yer tutucu bırak: `[Görsel: belge_id/dosya_adı]`. Yer tutucuyu künyedeki "
    "`görsel:` etiketinden birebir kopyala ve görselin metinde olması gereken noktaya yerleştir "
    "(görselin anlatıldığı bölümün hemen altına, ayrı bir satır olarak). Bir görseli cevabında "
    "yalnızca BİR kez göster: aynı görselden birden çok bilgi alınmışsa bile yer tutucuyu tek "
    "bir noktaya koy ve tekrarlama. Görseli cevabın altında ayrı bir `görsel:` veya 'Kaynaklar' "
    "listesiyle belirtme. Etiketi atlama, değiştirme veya cevabın sonuna iliştirme. Bu kural "
    "yalnızca görseller içindir; metin kaynaklar her bilgi için ayrı ayrı satır içi atıf alabilir."
)
